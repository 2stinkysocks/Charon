module.exports = {
    ...
    execute(message, channel, obols, fs, client, Discord, daily) {
        if(!daily && message.author.id != '456225381406605313' && message.author.id != '417439359868862465') return message.channel.send("You don't have permission to use this command!");
        var sosRole = channel.guild.roles.cache.find(role => role.name === "SoS").members;
        var alliesRole = channel.guild.roles.cache.find(role => role.name === "Allies").members;
        ...
        channel.send(memberList[randomMember][1].user.toString(), {embed});
    }   
}