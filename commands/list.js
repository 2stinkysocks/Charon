module.exports = {
    ...
    execute(message, args, oneListOnly) {
        if(args[0] == `count`) {
          ...
          if(args[0] != null) {
            if(message.guild.roles.cache.find(role => role.name == args[0].toLowerCase()) != null) {
              message.channel.send({embed: {
                color:4360181,
                title:`Members in ${message.guild.roles.cache.find(role => role.name == args[0].toLowerCase()).name}`,
                description: message.guild.roles.cache.find(role => role.name == args[0].toLowerCase()).members.map(m=>m.user.tag).join('\n')
              }});
              return;
            } else {
